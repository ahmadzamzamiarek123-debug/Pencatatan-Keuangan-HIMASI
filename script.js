...
const WEBAPP_URL = 'https://script.google.com/macros/s/AKfycbykdGmX8wT6Iox4eD_dDzKLLplQM6yAV3j988CO3orWwLQxYxSkBI_XFCD_cwrVXgGsBw/exec';
...
function switchTab(tab) {
  ...
  // Update tab buttons
  [el.tabMasuk, el.tabKeluar, el.tabRiwayat].forEach(t => t.classList.remove('active'));
  
  // Update panels
  [el.panelMasuk, el.panelKeluar, el.panelRiwayat].forEach(p => p.classList.remove('active'));
...
async function loadClasses() {
  try {
    ...
    if (data.success) {
      ...
      const options = classesData.map(k => `<option value="${k}">${k}</option>`).join('');
...
function renderMembers() {
  ...
  el.memberList.innerHTML = filteredMembers.map(m => `
...
function renderHistory() {
  ...
  const searchTerm = el.historySearch ? el.historySearch.value.toLowerCase() : '';
  if (searchTerm) {
  ...
  el.historyList.innerHTML = filtered.map(t => {
  ...
  const deleteType = t.type === 'out' ? 'keluar' : (t.type === 'in_member' ? 'masuk' : 'other');
      ...
      <div class="history-item type-${t.type}" data-id="${t.id}">
        ...
        <div class="history-footer">
          ...
          <div class="history-actions">
            ${t.type === 'in_member' ? `<button class="btn btn-sm btn-outline" onclick="openEditModal('${t.id}', 'masuk')">EDIT</button>` : ''}
            <button class="btn btn-sm btn-danger" onclick="openDeleteModal('${t.id}', '${deleteType}')">DEL</button>
          </div>
        </div>
      </div>
    `;
...
function getTypeLabel(type) {
  switch(type) {
    case 'in_member': return '🟢 IURAN';
    case 'in_other': return '🔵 LAIN';
...
function setupFilters() {
  el.filterKelas.addEventListener('change', () => loadMembers(el.filterKelas.value));
...
async function submitIncome() {
  ...
  try {
    await fetch(WEBAPP_URL, {
      ...
      body: JSON.stringify({ action: 'createIncome', secretKey: SECRET_KEY, transactions })
    });
    
    hideLoading();
    
    const total = transactions.reduce((s, t) => s + t.nominal, 0);
    showSuccess(`Data tersimpan!\n\nMinggu: ${week}\n${transactions.length} anggota\nTotal: ${formatRupiah(total)}`);
    resetMasukForm();
  } catch (err) {
    hideLoading();
...
async function submitExpense() {
  ...
  try {
    ...
    hideLoading();
    showSuccess(`Pengeluaran tersimpan!\n\n${kategori}\n${formatRupiah(nominal)}\nPJ: ${pj}`);
    resetKeluarForm();
  } catch (err) {
    hideLoading();
    showError('Cek Google Sheet untuk konfirmasi.');
...
async function submitOtherIncome() {
  ...
  try {
    ...
    hideLoading();
    showSuccess(`Pemasukan Lain tersimpan!\n\n${sumber}\n${formatRupiah(nominal)}\nPJ: ${pj}`);
...
async function saveEdit() {
  ...
  try {
    await fetch(WEBAPP_URL, {
      ...
      mode: 'no-cors',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ action: 'updateIncome', secretKey: SECRET_KEY, id: currentEditId, nominal })
    });
...
async function confirmDelete() {
  ...
  const action = currentDeleteType === 'masuk' ? 'deleteIncome' : 'deleteExpense';
  ...
  try {
    await fetch(WEBAPP_URL, {
      ...
      mode: 'no-cors',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ action, secretKey: SECRET_KEY, id: currentDeleteId })
    });
